...
function (context) {
  ...
  return [context.createToken("operator", ".", line, column), context.createToken("class", className, line, column + 1)];
},
...
function (context) {
  ...
  while (peek.length === count) {
    ...
    if (validHex && /[A-Fa-f0-9]/.test(letter)) value += letter;else validHex = false;
