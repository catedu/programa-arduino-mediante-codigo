...
exports.operators = exports.identAfterFirstLetter = exports.identFirstLetter = exports.customParseRules = exports.scopes = exports.name = undefined;
...
var name = exports.name = "nginx"; // sunlight-x: Intelligent Syntax Highlighting, Modernized
...
function () {
  var words = util.createHashMap(["server", "location", "upstream", "http", "mail", "types", "map", "split-clients", "geo", "limit_except"], "\\b", false);

  return function (context) {
    var token = util.matchWord(context, words, "context", true);
...
function (context) {
  ...
  while (walker.hasPrev()) {
    ...
    if (token.name === "operator" && util.contains(["^~", "~", "~*"], token.value)) {
      ...
      var prevToken = util.getPreviousWhile(context.getAllTokens(), walker.index, function (token) {
...
function () {
  var directives = util.createHashMap([
  // core: http://wiki.nginx.org/CoreModule
  "daemon", "env", "debug_points", "error_log", "include", "lock_file", "master_process", "pid", "ssl_engine", "timer_resolution", "user", "worker_cpu_affinity", "worker_priority", "worker_processes", "worker_rlimit_core", "worker_rlimit_nofile", "worker_rlimit_sigpending", "working_directory",

  // events: http://wiki.nginx.org/EventsModule
  "accept_mutext_delay", "accept_mutex", "debug_connection", "devpoll_changes", "devpoll_events", "kqueue_changes", "kqueue_events", "epoll_events", "multi_accept", "rtsig_signo", "rtsig_overflow_events", "rtsig_overflow_text", "rtsig_overflow_threshold", "use", "worker_connections",
  ...
  "ssl_certificate_key", "ssl_client_certificate", "ssl_certificate", "ssl_dhparam", "ssl_ciphers", "ssl_crl", "ssl_prefer_server_ciphers", "ssl_protocols", "ssl_verify_client", "ssl_verify_depth", "ssl_session_cache", "ssl_session_timeout", "ssl_engine", "ssl",
  ...
  "auth", "imap_capabilities", "imap_client_buffer", "listen", "pop3_auth", "pop3_capabilities", "protocol", "server", "server_name", "smtp_auth", "smtp_capabilities", "so_keepalive", "timeout",
  ...
  "echo_duplicate", "echo_flush", "echo_sleep", "echo_blocking_sleep", "echo_reset_timer", "echo_read_request_body", "echo_location_async", "echo_location", "echo_subrequest_async", "echo_subrequest", "echo_foreach_split", "echo_end", "echo_request_body", "echo_exec", "echo_before_body", "echo_after_body", "echo",

  // others
  "default", "output_buffers"], "[\\s;]", false);

  return function (context) {
    var token = util.matchWord(context, directives, "keyword", true);
    if (!token) return null;

    // must be the first word in a statement
    // which means first token in the string, or first non-ws token after "{", "}" or ";"
    var prevToken = util.getPreviousWhile(context.getAllTokens(), context.count(), function (token) {
      return token.name === "default" || token.name === "comment";
    });
